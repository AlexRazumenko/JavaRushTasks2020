...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        friendsSet.add(index);

        while (deep > 0) {
            for (Integer contact : friendsSet) {
                for (int i = 0; i < humanRelationships.length; i++) {
                    if ((i < contact) && humanRelationships[contact][i]) tempSet.add(i);
                    else if ((i > contact) && humanRelationships[i][contact]) tempSet.add(i);
                }
            }
